...
export default function WelcomePage({navigation}) {
  ...
  const setBudgetValue = (resp) => {
    if (resp && resp >= 0 && !resp.includes('.')) setIsValid(true);
    else setIsValid(false);
    wishHome.revenu_net = resp;
  }
  ...
  const handleSubmit = () => {
    if (isValid)
      navigation.navigate('WishHome_Demarche_Banque')
  ...
  return (
    <View style={container}>
      ...
      <View style={{padding: 20, height: '100%', justifyContent: "center"}}>
        ...
        <Title size="0" properties={{marginTop: 25, lineHeight: 50, width: "80%"}} title={`Veuillez saisir votre revenu net (toutes les personnes de votre foyer inclus)`}></Title>

        
        <TouchableOpacity style={InputProperties.element}>
          <Text style={InputProperties.label}>Veuillez saisir votre revenu net</Text>
          <TextInput onChangeText={setBudgetValue} inputMode="decimal" autoComplete="off"
