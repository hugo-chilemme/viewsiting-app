...
export default function WelcomePage({navigation}) {
  ...
  const setProfessionValue = (resp) => {
    if (resp.length > 2) setIsValid(true);
    else setIsValid(false);
    wishHome.profession = resp;
  }
  ...
  const handleSubmit = () => {
    if (isValid)
        if (wishHome.maried)
            navigation.navigate('WishHome_Maried_Identifier')
        else
            navigation.navigate('WishHome_Finance')
  ...
  return (
    <View style={container}>
      ...
      <View style={{padding: 20, height: '100%', justifyContent: "center"}}>
        ...
        <Title size="0" properties={{marginTop: 25, lineHeight: 50, width: "80%"}} title={`Veuillez saisir votre Profession`}></Title>

        
        <TouchableOpacity style={InputProperties.element}>
          <Text style={InputProperties.label}>Veuillez saisir votre Profession </Text>
          <TextInput onChangeText={setProfessionValue} inputMode="text" autoComplete="off"
